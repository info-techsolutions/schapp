           ...
           // if ($handle->uploaded) {
            ...
            //    $handle->process('../admin/pic/');
             ...
             //   if ($handle->processed) {
                    ...
                    $stmt = $db->prepare('INSERT INTO z_tb_person(person_number, firstname, lastname, gender, phone, address, class_id, arm_id, typ, payment_status) '
                            . 'VALUES (:person_number, :firstname, :lastname, :gender, :phone, :address, :class_id, :arm_id, :typ, :payment_status)');
                    ...
                    if($result){
                    ?>
                    <span class="flow-text">You have successfully added <?php echo $firstname . ' ' . $lastname ?> as a student <a href="show_student.php" class="ui small primary button" style="color: #ffffff; ">Home</a></span>

                    <?php
              } else {
                    // error occurred
              	    // $msg = $handle->error;
              	    $msg = 'Oopps! Unable to add student';
                    ?>
